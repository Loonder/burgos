        ...
        <div className="p-8 pb-32">
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-bold text-white flex items-center gap-3">
                    <Package className="text-burgos-primary" /> Loja de Produtos
                </h1>
                <button
                    onClick={() => {
                        setCurrentProduct({ name: '', description: '', price: 0, stock: 0, show_on_booking: true });
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {products.map(product => (
                    <div key={product.id} className="bg-burgos-secondary/20 rounded-xl border border-light-navy-100/10 overflow-hidden group">
                        <div className="h-40 bg-burgos-dark flex items-center justify-center">
                            {product.image ? (
                                <img src={product.image} alt={product.name} className="h-full w-full object-cover" />
                            ) : (
                                <Package size={48} className="text-burgos-accent/20" />
                            )}
                        </div>
                        <div className="p-4">
                            ...
                            <p className="text-burgos-accent/60 text-sm mb-3 line-clamp-2">{product.description}</p>
                            <div className="flex justify-between items-center">
                                <span className="text-lg font-bold text-burgos-primary">R$ {product.price?.toFixed(2)}</span>
                                <span className="text-xs text-burgos-accent/50">{product.stock} em estoque</span>
                            </div>
                        </div>
                    </div>
                ))}
